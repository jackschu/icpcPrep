...
int sumDivisors(vector<int> &p, int x) {
  ...
  if (x != 1)
    divs.push_back(pair<int, int>(x, 1));
  DEBUG for (auto elem : divs) printf("%d, %d\n", elem.first, elem.second);
  return getDivisors(divs, oldx);
}

// way too complicated approach, just checking all divisors up to sqrt is fast
// enough this is faster though since we get to divide x
int main() {
  int mx = 0;
  vector<int> xs;
  string l;
